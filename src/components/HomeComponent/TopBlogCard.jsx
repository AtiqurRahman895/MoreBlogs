...
const TopBlogCard = ({ blog,wishlistPage,setLoading }) => {
  return (
    <div className="bg-black">
      ...
      >
        <div className="p-4 h-fit bg-[rgba(0,0,0,0.5)] backdrop-blur-sm space-y-2 text-white text-center ">
          ...
          <h6 className={`font-Cinzel line-clamp-3 ${wishlistPage?"xl:text-xl":"xs:text-xl "} font-normal`}>{blog.title}</h6>
        </div>
        ...
        <div className="actionBtn bg-custom-half-primary backdrop-blur-[3px] absolute top-0 bottom-0 left-0 right-0 grid  items-center justify-center duration-500">
          <div className=" flex gap-4">
            <ReadThisBlogButton
              _id={wishlistPage?blog.blog_id:blog._id}
              buttonClass={"p-2"}
              iconClass={"text-2xl"}
            />
            {
